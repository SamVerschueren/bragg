...
class AWSApplication extends Application {
	createContext(req, ctx) {
		...
		} else if (req['http-method']) {
			Object.defineProperty(context, 'method', {enumerable: true, value: req['http-method']});
		}

		if (req.path) {
			Object.defineProperty(context, 'path', {enumerable: true, writable: true, value: req.path});
		} else if (req['resource-path']) {
			Object.defineProperty(context, 'path', {enumerable: true, writable: true, value: req['resource-path']});
		}
		...
		Object.defineProperty(context.request, 'query', {enumerable: true, writable: true, value: req.queryStringParameters || req.query || {}});
	...
	listen() {
		...
		return (req, context) => {
			...
			fn(ctx)
				...
				.catch(error => this.respond(context, ctx.onerror(error, ctx)));
		};
	}
}
